    ...
    <div className="fixed bottom-0 left-0 right-0 sm:bottom-24 sm:right-6 z-50 w-full sm:w-[360px] h-[50vh] sm:h-[480px] bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col border-t sm:border border-slate-200 overflow-hidden font-body animate-in slide-in-from-bottom-5 duration-200">
      ...
      <div className="px-4 py-2.5 sm:py-3 bg-slate-900 text-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-heading font-bold text-xs tracking-tight">Delivery Support</h3>
            <p className="text-[9px] text-slate-400">Direct operator connection</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-slate-50 space-y-3 scrollbar-thin">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center text-slate-400 space-y-2">
            ...
            <p className="text-[10px] max-w-[180px] text-slate-400">Coordinate pickup directions or request delivery updates.</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`flex gap-2 items-start ${isSelf ? 'flex-row-reverse' : 'flex-row'}`}
              >
                {/* Avatar Icon next to message */}
                <div className={`w-6 h-6 rounded-full shrink-0 flex items-center justify-center font-bold text-[9px] uppercase shadow-sm ${
                  isSelf ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-white'
                }`}>
                ...
                <div className={`flex flex-col ${isSelf ? 'items-end' : 'items-start'} max-w-[75%]`}>
                  <div
                    className={`rounded-2xl px-3.5 py-2 text-xs leading-normal shadow-sm font-medium ${
                      isSelf
